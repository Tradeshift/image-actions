...
jobs:
  build:
    ...
    steps:
      ...
      - name: Compress Images
        uses: tradeshift/image-actions@master
        with:
...
6. Confirm a successful Docker build with `docker build -t tradeshift/image-actions:dev .`.
7. [Open a Pull Request and follow the prompts](https://github.com/tradeshift/image-actions/compare).
